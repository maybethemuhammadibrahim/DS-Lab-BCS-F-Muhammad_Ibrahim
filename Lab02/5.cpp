...
void arithmetic(int **&resultant, int **&matrix1, int **&matrix2, int r1, int c1, bool isAdd)
{
    for (int i = 0; i < r1; i++)
    {
        for (int j = 0; j < c1; j++)
...
int main()
{
    ...
    cout << "Since this program only performs add/sub assuming rows and cols of both matrix are equal" << endl;
    ...
    enterData(matrix2, row1, col1);

    arithmetic(resultant, matrix1, matrix2, row1, col1, true);
    cout << "\nResult of Addition" << endl;
    display(resultant, row1, col1);
    arithmetic(resultant, matrix1, matrix2, row1, col1, false);
